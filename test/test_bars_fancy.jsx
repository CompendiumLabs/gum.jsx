...
return <Plot
  ...
  xticks={enumerate(labs)} yticks={linspace(0, 30, 7)} axis-tick-pos="outer" xaxis-line-lim={[0, 7]}
  xaxis-label-spin={-45} xaxis-label-justify="right" xaxis-label-loc={0.7} xaxis-label-offset={0.3}
>
  <Bars rounded={0.1} stroke={none} fill={blue} width={0.85}>{vals}</Bars>
  {vals.map((s, i) => <TextSpan pos={[i, s+1.5]} yrad={0.75}>{`${s}%`}</TextSpan> )}
</Plot>
