...
**Drawn shapes.** What no font carries is drawn: stretchy accents, extensible arrows and braces (`MathStretch`, `MathBrace`), the `\oiint` ring (`MathOval`), cancel strokes (`MathCancel`), rules and array lines (`MathRule`, `array_rules`). All of them extend `MathShape`, which rebases the context's stroke unit to the box's own pixels per em (so strokes are given in em) and resolves the colour with `shape_ink` — an explicit `fill`, else the `color` in force, else the theme's ink — so they follow the text in dark mode; `MathArray`'s rules and `enclose_box`'s border use the same rule. Composites built from placed items (`Frac`, `Sqrt`, `Accent`, `HorizBrace`, `Underline`/`Overline`) extend `MathGroup`, which adopts a `place_items` group's children, frame and math box. Delimiters follow TeX's rule in `fit_delim`: the first of Main, Size1…4 whose natural extent covers the requirement, unscaled, and only beyond Size4 a stretched glyph; `\big`…`\Bigg` feed katex's `sizeToMaxHeight` (1.2/1.8/2.4/3.0 em) into the same search.
