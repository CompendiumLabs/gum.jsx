...
- **Nested sizing** is relative to the size in force — `\tiny a \small b` sets b at 0.9, not 0.45 — tracked by a dynamically scoped `current_size` around the `sizing` body, which is safe only because element construction is synchronous.
