...
The rendering is also intentionally batched. **Points** handles the point cloud, while `Segments` draws the unit edges as a single SVG path instead of creating a separate **Line** element for every edge. That keeps the example responsive even though the graph has many repeated local hexagonal patterns.
